...
# add some noise np.random.normal(0, 0.02, 2)
...
# give field of view to fish
# weight cohesion/alignment by distance
# use n nearest neighbours (around 6 or 7 rather than all in radius) for alignment/cohesion
# blend together fleeing with boid movement
# readd limited turns
...
just never got triggered before. apply_boundary clips positions to [0, width]
inclusive, but Mesa's ContinuousSpace treats x >= width as out of bounds (strict < only). 
So any agent that tries to cross the edge gets clipped to exactly width, and then crashes
on its next move. Let me fix that:
...
class Fish(mesa.Agent):
    ...
    def alignment(self):
        ...
        for fish in self.nearby_fish:
            ...
            fish_heading = fish.velocity / magnitude
